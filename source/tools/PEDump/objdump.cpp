    ...
    )
{
    ...
    for (i = 0; i < count; i++)
    {
        if (
            !IsAddressInRange(
                (BYTE*) pRelocs,
                (int) sizeof(IMAGE_RELOCATION)
                )
            )
        {
            printf(
                "TODO: DumpObjRelocations %u of %u PIMAGE_RELOCATION out of range - %p\n",
                i,
                count,
                pRelocs
                );
        ...
        printf(
            "  Address: %08X  SymIndex: %08X  Type: %s\n",
...
//
void
DumpObjFile(PIMAGE_FILE_HEADER pImageFileHeader)
{
    unsigned i;
    int iret = 0;
    ...
    if (fShowRelocations)
    {
        ...
        if (max)
        {
                ...
                )
            {
                for (i = 0; i < max; i++)
                {
                        ...
                        )
                    {
                        ...
                        {
                            printf(
                                "TODO: Abandoned DumpObjRelocations() at %u of %u iterations...\n",
                                i,
                                max
                                );
                            break;
                    ...
                    {
                        printf(
                            "TODO: DUmpObjFile - Abandoned ShowRelocations at %u of %u iterations... PIMAGE_RELOCATION out of range - %p\n",
                            i,
                            max,
                            pir
                            );
            ...
            {
                printf(
                    "TODO: last PIMAGE_SECTION_HEADER out of range - %p\n",
                    last
                    );
            }
        }
    ...
    if (fShowSymbolTable && pImageFileHeader->PointerToSymbolTable)
    {
            ...
            )
        {
            ...
            {
                printf(
                    "TODO: DumpSymbolTable PSTR out of range - %p\n",
                    ps
                    );
            }
        }
        ...
        {
            printf(
                "TODO: DumpSymbolTable PIMAGE_SYMBOL out of range - %p\n",
                pv
                );
        }
                ...
                )
            {
                for (i = 0; i < wcnt; i++)
                {
                    ...
                    if (pSections->NumberOfLinenumbers)
                    {
                        ...
                        {
                            printf(
                                "TODO: Abandon ShowLineNumbers %u of %u - PIMAGE_LINENUMBER out of range - %p\n",
                                i,
                                wcnt,
                                pln
                                );
    ...
    if (fShowRawSectionData)
    {
        ...
        printf(
            "Show Raw Section Data: %u sections\n",
            nSects
            );
        ...
        {
            printf(
                "TODO: first PIMAGE_SECTION_HEADER out of range - %p\n",
                psh
                );
        }
    }
