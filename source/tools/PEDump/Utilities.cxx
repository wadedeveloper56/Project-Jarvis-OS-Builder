...
DiskType
is_file_or_directory32(const char* path)
{
    if (!path)
    {
        return MDT_NONE;
...
size_t
get_last_file_size32()
{
    return buf.st_size;
}

time_t
get_last_file_time32()
{
    return buf.st_mtime;
}
...
DiskType
is_file_or_directory64(const char* path)
{
    if (!path)
    {
        return MDT_NONE;
...
__int64
get_last_file_size64()
{
    return buf64.st_size;
}

time_t
get_last_file_time64()
{
...
char*
get_I64u_Stg(unsigned long long val)
{
    char* cp = GetNxtBuf();
#ifdef _MSC_VER
    sprintf(
