...
int
is_file_or_directory(const char* path)
{
    if (!path)
    {
        return MDT_NONE;
    ...
    if (len)
    {
        ...
        if (VFP(outfile))
        {
            ...
            if (w != len)
            {
                ...
                outfile = (FILE*) -1;
                sprtf(
                    "WARNING: Failed write to log file [%s] ...\n",
                    logfile
                    );
                exit(1);
            }
