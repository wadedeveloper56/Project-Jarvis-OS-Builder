...
//
BOOL
COFFSymbol::GetAuxSymbolAsString(
    PSTR pszBuffer,
    unsigned cbBuffer
    )
{
...
COFFSymbol::GetStorageClassName()
{
    ...
    if (
        (storageClass >= IMAGE_SYM_CLASS_BLOCK) &&
...
PCOFFSymbol
COFFSymbolTable::GetNearestSymbolFromRVA(
    DWORD rva,
    BOOL fExact
    )
{
