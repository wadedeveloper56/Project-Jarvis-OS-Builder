...
void arg_rex_errorfn(void* parent_, arg_dstr_t ds, int errorcode, const char* argval, const char* progname) {
    ArgRexPtr parent = parent_;
...
ArgRexPtr arg_rexn(const char* shortopts, const char* longopts, const char* pattern, const char* datatype, int mincount, int maxcount, int flags, const char* glossary) {
    ...
    nbytes = sizeof(ArgRex) + sizeof(struct privhdr)  + (size_t)maxcount * sizeof(char*);
