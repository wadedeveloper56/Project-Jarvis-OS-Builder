...
/***************/
{
    ...
    cur_posn = _lseek( Handle, 0, SEEK_CUR );
    if( cur_posn == -1L )
        return( true );
    eof_posn = _lseek( Handle, 0, SEEK_END );
    if( _lseek( Handle, cur_posn, SEEK_SET ) == -1L || eof_posn == -1L )
        return( true );
...
/*****************************************/
{
    ...
    if( amount > Sizeleft ) {
        ...
        if( amount/BSIZE >= 1 ) {
            Num_read = _read( Handle, &buffer[Sizeleft], amount );
            chkread( amount );
            Sizeleft = 0;
            Num_buff = FORCE_SEEK;  // must read in new buffer on next seek
        } else {
            Num_read = _read( Handle, Read_buff, BSIZE );
            chkread( amount );
...
/*********************************/
{
    ...
    nbuff = offset / BSIZE;
    if( nbuff != Num_buff ) {
        if( _lseek( Handle, nbuff * BSIZE, SEEK_SET ) == -1L ) {
...
/*****************************/
{
    ...
    cur_posn = _lseek( Handle, 0, SEEK_CUR );
    if( cur_posn != -1L ) {
        size = _lseek( Handle, 0, SEEK_END );
        _lseek( Handle, cur_posn, SEEK_SET );
    }
    return( size );
}
