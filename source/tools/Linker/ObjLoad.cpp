...
void loadlib(FILE* libfile, char * libname)
{
	...
	p->numsyms = numsyms;
	p->modsloaded = 0;
	p->modlist = (unsigned long*)checkMalloc(sizeof(unsigned short) * numsyms);
	libcount++;
}
...
void loadres(FILE* f)
{
	...
	while (!feof(f))
	{
		...
		else
		{
			...
			resource[rescount].typename0 = (unsigned char *)checkMalloc(j - i + 2);
