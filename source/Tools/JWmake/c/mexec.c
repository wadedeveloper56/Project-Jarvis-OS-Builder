...
/********************************/
{
    ...
    *p = NULLCHAR;                  /* terminate path */
    ...
    if( _chdir( path ) != 0 ) {      /* an error changing path */
...
#define DOMKDIR(d)  (_mkdir(d) == 0 || errno == EEXIST || errno == EACCES)
...
#else
            retcode = (int)_spawnvp( P_WAIT, cmdname, argv );
            if( retcode == -1 ) {
                PrtMsg( ERR | UNABLE_TO_EXEC, cmdname, strerror( errno ) );
            }
#endif
