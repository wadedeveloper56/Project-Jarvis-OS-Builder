...
static void peAddImport( arch_header *arch, libfile io )
{
    ...
    char            *currname = NULL;
    char            *buffer = NULL;
    Coff32_Export   *export_header = NULL;
    Coff32_EName    *name_table = NULL;
    Coff32_EOrd     *ord_table = NULL;
    int             i;
    long            ordinal_base;
    processor_type  processor = WL_PROC_NONE;
    importType      type;
...
void ProcessImport( char *name )
{
    ...
    /* syntax is: name.dllname.ordinal.ext_name */
    DEBUG(( "ProcessImport: %s\n", name ))
...
static void coffAddImportOverhead( arch_header *arch, char *DLLName, processor_type processor )
{
    ...
    DEBUG(("coffAddImportOverhead() enter\n"));
    buffer = MemAlloc( 100 );
...
int CoffImportSize( import_sym *import )
{
    ...
    case NAMED:
...
#if 1 /* JWLink */
            if ( import->u.sym.symName )
                ret = sizeof( coff_import_object_header ) + sym_len + 1 + dll_len + 1;
            else
                ret = sizeof( coff_import_object_header ) + exp_len + 1 + dll_len + 1;
#else
		...
		//printf("CoffImportSize: %s, size=%x\n", import->u.sym.symName, ret );
#if 0
