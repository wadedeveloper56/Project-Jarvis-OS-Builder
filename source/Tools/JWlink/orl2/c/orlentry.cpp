...
orl_file_handle ORLFileInit(orl_handle orl_hnd, void* file, orl_file_format type)
{
...
unsigned long ORLExportTableRVA(orl_file_handle orl_file_hnd)
{
...
orl_return ORLFileScan(orl_file_handle orl_file_hnd, unsigned long desired, orl_sec_return_func return_func)
{
	switch (orl_file_hnd->type)
	{
		case(ORL_ELF):
...
char* ORLSecGetName(orl_sec_handle orl_sec_hnd)
{
...
orl_sec_offset ORLSecGetBase(orl_sec_handle orl_sec_hnd)
{
	switch (orl_sec_hnd->type)
	{
		case(ORL_ELF):
...
char* ORLSecGetClassName(orl_sec_handle orl_sec_hnd)
{
...
orl_return ORLSecGetContents(orl_sec_handle orl_sec_hnd, char** buffer)
{
	switch (orl_sec_hnd->type)
	{
		case(ORL_ELF):
...
orl_table_index ORLCvtSecHdlToIdx(orl_sec_handle shdl)
{
	switch (shdl->type)
	{
		case(ORL_ELF):
...
orl_sec_handle ORLCvtIdxToSecHdl(orl_file_handle fhdl,
	orl_table_index idx)
{
	switch (fhdl->type)
	{
...
orl_return ORLNoteSecScan(orl_sec_handle orl_sec_hnd,
...
char* ORLSymbolGetName(orl_symbol_handle orl_symbol_hnd)
{
...
orl_symbol_value ORLSymbolGetValue(orl_symbol_handle orl_symbol_hnd)
{
	switch (orl_symbol_hnd->type)
	{
		case(ORL_ELF):
...
orl_symbol_handle ORLSymbolGetAssociated(orl_symbol_handle hnd)
{
	switch (hnd->type)
	{
		case(ORL_ELF):
...
orl_return ORLGroupsScan(orl_file_handle hnd,
...
char* ORLGroupName(orl_group_handle hnd)
{
	if (hnd->type == ORL_OMF)
	{
...
orl_table_index ORLGroupSize(orl_group_handle hnd)
{
	if (hnd->type == ORL_OMF)
	{
...
char* ORLGroupMember(orl_group_handle hnd, orl_table_index idx)
{
	if (hnd->type == ORL_OMF)
	{
