...
static void determine_file_class( elf_file_handle elf_file_hnd, Elf32_Ehdr *e_hdr )
{
    ...
    case ELFLITTLEENDIAN:
        BS_ORE(orl_file_flags, elf_file_hnd->flags, |=, ORL_FILE_FLAG_LITTLE_ENDIAN);
        break;
    case ELFBIGENDIAN:
        BS_ORE(orl_file_flags, elf_file_hnd->flags, |=, ORL_FILE_FLAG_BIG_ENDIAN);
        break;
    }
    switch( e_hdr->e_ident[EI_CLASS] ) {
    case ELFCLASS32:
        BS_ORE(orl_file_flags, elf_file_hnd->flags, |=, ORL_FILE_FLAG_32BIT_MACHINE);
        break;
    case ELFCLASS64:
        BS_ORE(orl_file_flags, elf_file_hnd->flags, |=, ORL_FILE_FLAG_64BIT_MACHINE);
        break;
    }
}
                                     ...
                                     unsigned long long sh_flags )
{
    ...
    case SHT_PROGBITS:
        ...
        BS_ORE(orl_sec_flags, elf_sec_hnd->flags, |=, ORL_SEC_FLAG_INITIALIZED_DATA);
        break;
    case SHT_SYMTAB:
    ...
    case SHT_NOBITS:
        ...
        BS_ORE(orl_sec_flags, elf_sec_hnd->flags, |=, ORL_SEC_FLAG_UNINITIALIZED_DATA);
        break;
    case SHT_REL:
    ...
    if( sh_flags & SHF_WRITE ) {
        BS_ORE(orl_sec_flags, elf_sec_hnd->flags, |=, ORL_SEC_FLAG_WRITE_PERMISSION);
    }
    if( !(sh_flags & SHF_ALLOC) ) {
        BS_ORE(orl_sec_flags, elf_sec_hnd->flags, |=, ORL_SEC_FLAG_REMOVE)  ;
    }
    if( sh_flags & SHF_EXECINSTR ) {
        // set execute permission also?
        BS_ORE(orl_sec_flags, elf_sec_hnd->flags, |=, ORL_SEC_FLAG_EXEC);
    }
}
...
orl_return ElfLoadFileStructure( elf_file_handle elf_file_hnd )
{
    ...
    _ClientSeek( elf_file_hnd, - (long long)sizeof( e_hdr32->e_ident ), SEEK_CUR );
    if( elf_file_hnd->flags & ORL_FILE_FLAG_64BIT_MACHINE ) {
