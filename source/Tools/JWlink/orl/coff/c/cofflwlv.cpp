    ...
    for( loop = 0; loop < file_hnd->num_symbols; loop++ ) {
        ...
        if( current->symbol->name.non_name.zeros == 0 ) {
            ...
            if( strlen( current->symbol->name.name_string ) >= COFF_SYM_NAME_LEN ) {
                ...
                current->name = (char *) current->symbol->name.name_string;
                current->name_alloced = COFF_FALSE;
            }
        }
        ...
        switch( current->symbol->sec_num ) {
            case IMAGE_SYM_DEBUG:
                BS_ORE(orl_symbol_type, current->type, |=, ORL_SYM_TYPE_DEBUG);
                current->binding = ORL_SYM_BINDING_NONE;
                break;
            case IMAGE_SYM_ABSOLUTE:
                BS_ORE(orl_symbol_type, current->type, |=, ORL_SYM_TYPE_ABSOLUTE);
                current->binding = ORL_SYM_BINDING_NONE; // ?
                break;
            case IMAGE_SYM_UNDEFINED:
                if( current->symbol->value == 0) {
                    BS_ORE(orl_symbol_type, current->type, |=, ORL_SYM_TYPE_UNDEFINED);
                } else {
                    BS_ORE(orl_symbol_type, current->type, |=, ORL_SYM_TYPE_COMMON);
                }
                break;
            default:
                BS_ORE(orl_symbol_type, current->type, |=, ORL_SYM_TYPE_DEFINED);
                sechdl = file_hnd->orig_sec_hnd[current->symbol->sec_num - 1];
                if( sechdl->flags & ORL_SEC_FLAG_COMDAT ) {
                    BS_ORE(orl_symbol_type, current->type, |=, ORL_SYM_CDAT_MASK);
                }
                break;
        }
        switch( current->symbol->storage_class ) {
            ...
            case IMAGE_SYM_CLASS_WEAK_EXTERNAL:
                ...
                if( type & IMAGE_SYM_DTYPE_FUNCTION ) {
                    BS_ORE(orl_symbol_type, current->type, |=, ORL_SYM_TYPE_FUNCTION);
                } else {
                    BS_ORE(orl_symbol_type, current->type, |=, ORL_SYM_TYPE_OBJECT);
                }
                break;
            case IMAGE_SYM_CLASS_STATIC:
                ...
                if( current->symbol->num_aux == 0 ) {
                    if( sechdl != NULL
                        ...
                        BS_ORE(orl_symbol_type, current->type, |=, ORL_SYM_TYPE_SECTION);
                    } else {
                        type = _CoffComplexType( current->symbol->type );
                        if( type & IMAGE_SYM_DTYPE_FUNCTION ) {
                            BS_ORE(orl_symbol_type, current->type, |=, ORL_SYM_TYPE_FUNCTION);
                    ...
                    BS_ORE(orl_symbol_type, current->type, |=, (aux->selection << ORL_SYM_CDAT_SHIFT) & ORL_SYM_CDAT_MASK);
                } else {
                    type = _CoffComplexType( current->symbol->type );
                    if( type & IMAGE_SYM_DTYPE_FUNCTION ) {
                        BS_ORE(orl_symbol_type, current->type, |=, ORL_SYM_TYPE_FUNCTION);
            ...
            case IMAGE_SYM_CLASS_FILE:
                current->binding = ORL_SYM_BINDING_LOCAL;
                BS_ORE(orl_symbol_type, current->type, |=, ORL_SYM_TYPE_FILE);
                break;
        }
        prev = loop;
    ...
    if( coff_file_hnd->machine_type == ORL_MACHINE_TYPE_ALPHA ) {
        ...
        switch( coff_type ) {
            ...
            case IMAGE_REL_AMD64_ADDR32:
                return( ORL_RELOC_TYPE_WORD_32 );
#if 1  /* added for wlink 64-bit support */
...
static orl_return ParseExport( char **contents, int *len,
    orl_return  (*callback_fn)( char *, void * ), void *cookie )
/**************************************************************/
{
    char *      arg;
    int         l;
...
/*******************************************************************/
{
    ...
    while( len > 0 ) {
        /* jwlink: MSVC options may start with '/'! */
