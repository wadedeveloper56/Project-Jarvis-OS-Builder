...
/**************************************************************/
{
    ...
    if( flags & IMAGE_FILE_RELOCS_STRIPPED ) {
            BS_ORE(orl_file_flags, coff_file_hnd->flags, |=, ORL_FILE_FLAG_RELOCS_STRIPPED);
    }
    if( flags & IMAGE_FILE_LINE_NUMS_STRIPPED ) {
            BS_ORE(orl_file_flags, coff_file_hnd->flags, |=, ORL_FILE_FLAG_LINE_NUMS_STRIPPED);
    }
    if( flags & IMAGE_FILE_LOCAL_SYMS_STRIPPED ) {
            BS_ORE(orl_file_flags, coff_file_hnd->flags, |=, ORL_FILE_FLAG_LOCAL_SYMS_STRIPPED);
    }
    if( flags & IMAGE_FILE_DEBUG_STRIPPED ) {
            BS_ORE(orl_file_flags, coff_file_hnd->flags, |=, ORL_FILE_FLAG_DEBUG_STRIPPED);
    }
    if( flags & IMAGE_FILE_16BIT_MACHINE ) {
            BS_ORE(orl_file_flags, coff_file_hnd->flags, |=, ORL_FILE_FLAG_16BIT_MACHINE);
    }
    if( flags & IMAGE_FILE_32BIT_MACHINE ) {
            BS_ORE(orl_file_flags, coff_file_hnd->flags, |=, ORL_FILE_FLAG_32BIT_MACHINE);
    }
    if( flags & IMAGE_FILE_SYSTEM ) {
            BS_ORE(orl_file_flags, coff_file_hnd->flags, |=, ORL_FILE_FLAG_SYSTEM);
    }
    if( coff_file_hnd->type != ORL_FILE_TYPE_OBJECT ) {
         ...
         */
        BS_ORE(orl_file_flags, coff_file_hnd->flags, |=, ORL_FILE_FLAG_LITTLE_ENDIAN);  
    } else {
        /*
        ...
        */
        if( flags & IMAGE_FILE_BYTES_REVERSED_HI ) {
            BS_ORE(orl_file_flags, coff_file_hnd->flags, |=, ORL_FILE_FLAG_BIG_ENDIAN);
        } else {
...
/****************************************************************/
{
    ...
    if( s_hdr->flags & IMAGE_SCN_LNK_INFO ) {
        ...
        BS_ORE(orl_sec_flags, coff_sec_hnd->flags, |=, ORL_SEC_FLAG_EXEC);
    } else if( s_hdr->flags & IMAGE_SCN_CNT_INITIALIZED_DATA ) {
        coff_sec_hnd->type = ORL_SEC_TYPE_PROG_BITS;
        BS_ORE(orl_sec_flags, coff_sec_hnd->flags, |=, ORL_SEC_FLAG_INITIALIZED_DATA);
    } else if( s_hdr->flags & IMAGE_SCN_CNT_UNINITIALIZED_DATA ) {
        coff_sec_hnd->type = ORL_SEC_TYPE_PROG_BITS;
        BS_ORE(orl_sec_flags, coff_sec_hnd->flags, |=, ORL_SEC_FLAG_UNINITIALIZED_DATA);
    } else {
...
//  }
    if( s_hdr->flags & IMAGE_SCN_TYPE_NO_PAD ) {
        BS_ORE(orl_sec_flags, coff_sec_hnd->flags, |=, ORL_SEC_FLAG_NO_PADDING);
    }
...
//  }
    if( s_hdr->flags & IMAGE_SCN_LNK_REMOVE ) {
        BS_ORE(orl_sec_flags, coff_sec_hnd->flags, |=, ORL_SEC_FLAG_REMOVE);
    }
    if( s_hdr->flags & IMAGE_SCN_LNK_COMDAT ) {
        BS_ORE(orl_sec_flags, coff_sec_hnd->flags, |=, ORL_SEC_FLAG_COMDAT);
    }
    if( s_hdr->flags & IMAGE_SCN_MEM_DISCARDABLE ) {
        BS_ORE(orl_sec_flags, coff_sec_hnd->flags, |=, ORL_SEC_FLAG_DISCARDABLE);
    }
    if( s_hdr->flags & IMAGE_SCN_MEM_NOT_CACHED ) {
        BS_ORE(orl_sec_flags, coff_sec_hnd->flags, |=, ORL_SEC_FLAG_NOT_CACHED);
    }
    if( s_hdr->flags & IMAGE_SCN_MEM_NOT_PAGED ) {
        BS_ORE(orl_sec_flags, coff_sec_hnd->flags, |=, ORL_SEC_FLAG_NOT_PAGEABLE);
    }
    if( s_hdr->flags & IMAGE_SCN_MEM_SHARED ) {
        BS_ORE(orl_sec_flags, coff_sec_hnd->flags, |=, ORL_SEC_FLAG_SHARED);
    }
    if( s_hdr->flags & IMAGE_SCN_MEM_EXECUTE ) {
        BS_ORE(orl_sec_flags, coff_sec_hnd->flags, |=, ORL_SEC_FLAG_EXECUTE_PERMISSION);
    }
    if( s_hdr->flags & IMAGE_SCN_MEM_READ ) {
        BS_ORE(orl_sec_flags, coff_sec_hnd->flags, |=, ORL_SEC_FLAG_READ_PERMISSION);
    }
    if( s_hdr->flags & IMAGE_SCN_MEM_WRITE ) {
        BS_ORE(orl_sec_flags, coff_sec_hnd->flags, |=, ORL_SEC_FLAG_WRITE_PERMISSION);
    }
