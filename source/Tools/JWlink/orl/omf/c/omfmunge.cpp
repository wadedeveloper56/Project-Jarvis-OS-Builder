                                          ...
                                          omf_symbol_handle sym )
{
    ...
    if( !sh ) {
        ...
        ofh->symbol_table = sh;
        BS_ORE(orl_sec_flags, sh->flags, |=, ORL_SEC_FLAG_REMOVE);
...
static orl_return       addReloc( omf_file_handle ofh, omf_reloc_handle orh )
{
    ...
    if( !ofh->relocs ) {
        ...
        BS_ORE(orl_sec_flags, ofh->relocs->flags, |=, ORL_SEC_FLAG_REMOVE);
    }
...
static orl_sec_flags    getSegSecFlags( omf_file_handle ofh, omf_idx name,
                                        omf_idx class1, orl_sec_alignment align,
                                        int combine, int use32 )
{
    ...
    slen = OmfGetLName( ofh->lnames, class1, lname );
    if( slen > 0 ) {
        lname[slen] = '\0';
        ...
        if( ( slen > 3 ) &&
            ...
            BS_ORE(orl_sec_flags, flags, |=, ORL_SEC_FLAG_EXEC | ORL_SEC_FLAG_EXECUTE_PERMISSION | ORL_SEC_FLAG_READ_PERMISSION);
        } else if( ( slen > 3 ) && !strcmp( "DATA", &lname[slen - 4] ) ) {
            BS_ORE(orl_sec_flags, flags, |=, ORL_SEC_FLAG_READ_PERMISSION);
            slen = OmfGetLName( ofh->lnames, name, lname );
            if( slen > 0 ) {
                lname[slen] = '\0';
                strNUpper( lname, slen );
                if( strstr( "CONST", lname ) ) {
                        BS_ORE(orl_sec_flags, flags, |=, ORL_SEC_FLAG_READ_PERMISSION | ORL_SEC_FLAG_INITIALIZED_DATA); 
                }
            }
        } else if( ( slen > 4 ) && !strcmp( "CONST", &lname[slen - 5] ) ) {
            BS_ORE(orl_sec_flags, flags, |=, ORL_SEC_FLAG_READ_PERMISSION | ORL_SEC_FLAG_INITIALIZED_DATA);
        } else if( ( slen > 2 ) && !strcmp( "BSS", &lname[slen - 3] ) ) {
            BS_ORE(orl_sec_flags, flags, |=, ORL_SEC_FLAG_READ_PERMISSION | ORL_SEC_FLAG_WRITE_PERMISSION | ORL_SEC_FLAG_UNINITIALIZED_DATA);
            ...
            if( ( slen > 3 ) &&
                ( !strcmp( "CODE", &lname[slen - 4] ) ||
                  !strcmp( "TEXT", &lname[slen - 4] ) ) ) {
                BS_ORE(orl_sec_flags, flags, |=, ORL_SEC_FLAG_EXEC | ORL_SEC_FLAG_EXECUTE_PERMISSION);
            }
        }
    } else {
        BS_ORE(orl_sec_flags, flags, |=, ORL_SEC_FLAG_READ_PERMISSION);
    ...
    if( use32 ) {
        BS_ORE(orl_sec_flags, flags, |=, ORL_SEC_FLAG_USE_32);
    }
                                      ...
                                      omf_bytes buffer, long len, int comdat )
{
     ...
     */
    if( !ofh->lidata ) {
        ofh->lidata = (omf_tmp_lidata_struct *)_ClientAlloc( ofh, sizeof( omf_tmp_lidata_struct ) );
                                      ...
                                      unsigned int len, omf_rectyp typ )
{
    ...
    if( ( typ == CMD_COMDEF ) || ( typ == CMD_LCOMDEF ) ) {
        BS_ORE(orl_symbol_type, styp, |=, ORL_SYM_TYPE_COMMON);
    } else {
        BS_ORE(orl_symbol_type, styp, |=, ORL_SYM_TYPE_UNDEFINED);
    }
                                      ...
                                      omf_rectyp typ )
{
    ...
    if( flags & COMDAT_CONTINUE ) {
        ...
        BS_ORE(orl_sec_flags, sh->flags, |=, ORL_SEC_FLAG_COMDAT | ORL_SEC_FLAG_READ_PERMISSION | ORL_SEC_FLAG_INITIALIZED_DATA);
        ...
        case( COMDAT_EXPLICIT ):        /* in given segment */
            ...
            BS_ORE(orl_sec_flags, sh->flags, |=, sh->assoc.seg.comdat.assoc_seg->flags);
            BS_ORE(orl_sec_combine, sh->assoc.seg.combine, |=, ORL_SEC_COMBINE_COMDAT_ALLOC_EXPLIC);
            if( group ) {
                sh->assoc.seg.comdat.group = findGroup( ofh, group );
            }
            break;
        case( COMDAT_FAR_CODE ):        /* allocate CODE use16 segment */
            BS_ORE(orl_sec_flags, sh->flags, |=, ORL_SEC_FLAG_EXEC | ORL_SEC_FLAG_EXECUTE_PERMISSION);
            BS_ORE(orl_sec_combine, sh->assoc.seg.combine, |=, ORL_SEC_COMBINE_COMDAT_ALLOC_CODE16);
            break;
        case( COMDAT_CODE32 ):          /* allocate CODE use32 segment */
            BS_ORE(orl_sec_flags, sh->flags, |=, ORL_SEC_FLAG_EXEC | ORL_SEC_FLAG_EXECUTE_PERMISSION);
            BS_ORE(orl_sec_combine, sh->assoc.seg.combine, |=, ORL_SEC_COMBINE_COMDAT_ALLOC_CODE32);
            break;
        case( COMDAT_FAR_DATA ):        /* allocate DATA use16 segment */
            BS_ORE(orl_sec_flags, sh->flags, |=, ORL_SEC_FLAG_WRITE_PERMISSION);
            BS_ORE(orl_sec_combine, sh->assoc.seg.combine, |=, ORL_SEC_COMBINE_COMDAT_ALLOC_DATA16);
            break;
        case( COMDAT_DATA32 ):          /* allocate DATA use32 segment */
            BS_ORE(orl_sec_flags, sh->flags, |=, ORL_SEC_FLAG_WRITE_PERMISSION);
            BS_ORE(orl_sec_combine, sh->assoc.seg.combine, |=, ORL_SEC_COMBINE_COMDAT_ALLOC_DATA32);
            break;
        }
        ...
        case( COMDAT_MATCH_NONE ):              /* don't match anyone */
            BS_ORE(orl_symbol_type, styp, |=, ORL_SYM_CDAT_NODUPLICATES);
            BS_ORE(orl_sec_combine, sh->assoc.seg.combine, |=, ORL_SEC_COMBINE_COMDAT_PICK_NONE);
            break;
        case( COMDAT_MATCH_ANY ):               /* pick any instance */
            BS_ORE(orl_symbol_type, styp, |=, ORL_SYM_CDAT_ANY);
            BS_ORE(orl_sec_combine, sh->assoc.seg.combine, |=, ORL_SEC_COMBINE_COMDAT_PICK_ANY);
            break;
        case( COMDAT_MATCH_SAME ):              /* must be same size */
            BS_ORE(orl_symbol_type, styp, |=, ORL_SYM_CDAT_SAME_SIZE);
            BS_ORE(orl_sec_combine, sh->assoc.seg.combine, |=, ORL_SEC_COMBINE_COMDAT_PICK_SAME);
            break;
        case( COMDAT_MATCH_EXACT ):             /* must be exact match */
            BS_ORE(orl_symbol_type, styp, |=, ORL_SYM_CDAT_EXACT);
            BS_ORE(orl_sec_combine, sh->assoc.seg.combine, |=, ORL_SEC_COMBINE_COMDAT_PICK_EXACT);
            break;
        }
                                      ...
                                      omf_idx class1 )
{
    ...
    BS_ORE(orl_sec_flags, sh->flags, |=, getSegSecFlags(ofh, name, class1, align, combine, use32));
                                      ...
                                      orl_sec_offset offset, omf_rectyp typ )
{
    ...
    BS_ORE(orl_symbol_type, styp, |=, ORL_SYM_TYPE_OBJECT);
                                      ...
                                      omf_idx *segs, int size )
{
    ...
    while( size ) {
        ...
        if( !sh ) return( ORL_ERROR );
        BS_ORE(orl_sec_flags, sh->flags, |=, ORL_SEC_FLAG_GROUPED);
