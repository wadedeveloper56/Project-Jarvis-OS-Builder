...
/************************************************/
{
    ...
    first = TRUE;
    InVerifySegment = FALSE;
    /* _write all _read/_write segment data */
    ...
    /* _write all _read only segment data */
    for( grp = Groups; grp != NULL; grp = grp->next_group ){
        if( grp->u.qnxflags != QNX_READ_WRITE && grp->totalsize != 0 ) {
            WriteQNXGroup( grp, segments );
        }
...
// specified.
    ...
    if( sflags < 0x10 ) {
        if( !(sflags & 1) ) {     // if can _read/_write or exec/_read
            grp->u.qnxflags &= ~1;       // can for all segments.
            return TRUE;                // no need to check others
        }
    } else {
// make segments _read/_write or exec/_read unless every segment is specifically
// set otherwise.
        grp->u.qnxflags &= ~1;
        return TRUE;
    }
