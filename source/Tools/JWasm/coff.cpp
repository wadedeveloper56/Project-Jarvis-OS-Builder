...
/********************************************************************************/
{
	...
	name = (struct stringitem*)LclAlloc(len + sizeof(struct stringitem));
	name->next = NULL;
...
/***********************************************/
{
	...
	for (i = 0, curr = (struct line_num_info*)q->head; curr; i++, curr = curr->next);
	return(i);
}
...
/*****************************************************************************************/
{
	...
	for (curr = SymTables[TAB_SEG].head; curr; curr = curr->next)
	{
		...
		if (curr->e.seginfo->LinnumQueue)
		{
			...
			ish.NumberOfLinenumbers = GetLinnumItems((struct qdesc *)curr->e.seginfo->LinnumQueue);
...
/********************************************************************************/
{
	...
	if (modinfo->g.SafeSEHQueue.head)
	{
		...
		struct qnode* sehp2;
		uint_32* pdw;
		for (sehp = (struct qnode*)modinfo->g.SafeSEHQueue.head, pdw = (uint_32*)cm->sxdata->e.seginfo->CodeBuffer; sehp; sehp = sehp2)
		{
			sehp2 = (struct qnode*)sehp->next;
...
/********************************************************************************/
{
		...
		exp != NULL)
	{
		if ((cm->directives = (struct dsym*)CreateIntSegment(szdrectve, "", MAX_SEGALIGNMENT, modinfo->Ofssize, FALSE)) != NULL)
		{
			...
			/* 2. defaultlibs */
			for (q = (struct qitem*)modinfo->g.LibQueue.head; q; q = (struct qitem*)q->next)
			{
			...
			cm->directives->e.seginfo->CodeBuffer = (uint_8*)LclAlloc(size + 1);
			...
			for (tmp = imp; tmp; tmp = tmp->next)
			{
					...
					tmp->sym.dll && *tmp->sym.dll->name)
				{
					...
					*p++ = '=';
					...
					p += strlen((char*)p);
					*p++ = '.';
					memcpy((char*)p, tmp->sym.name, tmp->sym.name_size);
					p += tmp->sym.name_size;
					*p++ = ' ';
				}
			}
...
/**************************************************************/
{
		...
		if (i == DBGS_MAX)
		{
			...
			for (i = 0; i < DBGS_MAX; i++)
			{
				...
				if (cm.SymDeb[i].seg->sym.max_offset)
				{
					...
					for (src = (uint_8*)cm.SymDeb[i].q.head; src; src = (uint_8*)((struct qditem*)src)->next)
					{
	...
	if (modinfo->g.SafeSEHQueue.head)
	{
		...
		if ((cm.sxdata = (struct dsym*)CreateIntSegment(".sxdata", "", MAX_SEGALIGNMENT, modinfo->Ofssize, FALSE)) != NULL)
		{
			...
			/* calc the size for this segment */
			for (sehp = (struct qnode*)modinfo->g.SafeSEHQueue.head; sehp; sehp = (struct qnode*)sehp->next, cnt++);
			cm.sxdata->sym.max_offset = cnt * 4;
			cm.sxdata->e.seginfo->CodeBuffer = (uint_8*)LclAlloc(cnt * 4);
