...
Macro::Macro(vector<string>& f) : nameTokDef(nullptr), valueToken(nullptr), endToken(nullptr), files(f), tokenListDefine(f), variadic(false), variadicOpt(false), optExpandValue(nullptr), optNoExpandValue(nullptr), valueDefinedInCode_(false)
{}

Macro::Macro(const Token* tok, vector<string>& f) : nameTokDef(nullptr), files(f), tokenListDefine(f), valueDefinedInCode_(true) {
...
Macro::Macro(const string& name, const string& value, vector<string>& f) : nameTokDef(nullptr), files(f), tokenListDefine(f), valueDefinedInCode_(false) {
...
const Token* Macro::expand(TokenList* const output, const Token* rawtok, const MacroMap& macros, vector<string>& inputFiles) const {
...
const list<Location>& Macro::usage() const {
	return usageList;
}
...
vector<const Token*> Macro::getMacroParameters(const Token* nameTokInst, bool calledInDefine) const {
...
const Token* Macro::expand(TokenList* const output, const Location& loc, const Token* const nameTokInst, const MacroMap& macros, set<TokenString> expandedmacros) const {
...
const Token* Macro::recursiveExpandToken(TokenList* output, TokenList& temp, const Location& loc, const Token* tok, const MacroMap& macros, const set<TokenString>& expandedmacros, const vector<const Token*>& parametertokens) const {
...
const Token* Macro::expandToken(TokenList* output, const Location& loc, const Token* tok, const MacroMap& macros, const set<TokenString>& expandedmacros, const vector<const Token*>& parametertokens) const {
	// Not name..
	if (!tok->name)
	{
...
bool Macro::expandArg(TokenList* output, const Token* tok, const vector<const Token*>& parametertokens) const {
	if (!tok->name)
		return false;
...
bool Macro::expandArg(TokenList* output, const Token* tok, const Location& loc, const MacroMap& macros, const set<TokenString>& expandedmacros, const vector<const Token*>& parametertokens) const {
	if (!tok->name)
		return false;
	const unsigned int argnr = getArgNum(tok->str());
	if (argnr >= args.size())
...
const Token* Macro::expandHash(TokenList* output, const Location& loc, const Token* tok, const set<TokenString>& expandedmacros, const vector<const Token*>& parametertokens) const {
...
const Token* Macro::expandHashHash(TokenList* output, const Location& loc, const Token* tok, const MacroMap& macros, const set<TokenString>& expandedmacros, const vector<const Token*>& parametertokens, bool expandResult) const {
	Token* A = output->back();
	if (!A)
...
bool Macro::isReplaced(const set<string>& expandedmacros) {
