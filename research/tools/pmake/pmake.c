...
static int _comparison( const void *pp1, const void *pp2, bool reverse )
{
    ...
    if( p1->depth < p2->depth )
        return( +1 );
    return( _stricmp( p1->dir_name, p2->dir_name ) );
}
...
static priority_type CompareTargets( target_list *targets, const char *line )
{
    ...
    line++;
    SKIP_SPACES( line );
    if( _strnicmp( line, COOKIE, sizeof( COOKIE ) - 1 ) != 0 )
        return( NONE_PRIORITY );
    ...
    while( *line != '\0' ) {
        for( target = targets; target != NULL; target = target->next ) {
            if( target->flags == TARGET_NOT_USED ) {
                ...
                if( _strnicmp( line, target->string, len ) == 0 ) {
...
static void NextSubdir( void )
{
    ...
    while( QueueHead->next != NULL ) {
        if( last_ok == NULL ) {
            if( _chdir( RelativePath( QueueHead->name, QueueHead->next->name ) ) == 0 )
                break;
            last_ok = QueueHead;
            QueueHead = QueueHead->next;
        } else if( _chdir( RelativePath( last_ok->name, QueueHead->next->name ) ) == 0 ) {
            MFree( last_ok );
            break;
        } else {
            DeQueue();
    ...
    while( QueueHead != NULL ) {
        ...
        if( dirp != NULL ) {
            while( (dire = readdir( dirp )) != NULL ) {
                ...
                } else if( _stricmp( dire->d_name, data->makefile ) == 0 ) {
...
pmake_data *PMakeBuild( pmake_data *data, const char *cmd )
{
    ...
    if( _getcwd( SaveDir, _MAX_PATH ) != NULL ) {
        DoneFlag = false;
