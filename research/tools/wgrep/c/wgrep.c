        ...
        if( dirp != NULL ) {
            ...
            for( ; !DoneFlag && (dire = readdir( dirp )) != NULL; ) {
                ...
                if( IgnoreListCnt > 0 ) {
                    ...
                    if( _stricmp( pg1.ext, pg2.ext ) != 0 ) {
                        for( i = 0; i < IgnoreListCnt; i++ ) {
                            if( _stricmp( pg2.ext, IgnoreList[i] ) == 0 ) {
                                break;
                            }
                        }
                        if( i < IgnoreListCnt ) {
...
static char **parseSearchStrings( char **arg )
{
    ...
    while( *arg != NULL && **arg == '-' && arg[0][1] == 'e' && i < MAX_SRCH_STRINGS ) {
        if( arg[0][2] == '\0' ) {
            if( arg[1] != NULL ) {
                SrchStrings[i] = _strdup( arg[1] );
            } else {
                Error( "Missing search string", "" );
            }
            arg += 2;
        } else {
            SrchStrings[i] = _strdup( *arg + 2 );
            arg++;
        }
    ...
    while( *arg != NULL && **arg == '@' ) {
        ...
        while( i < MAX_SRCH_STRINGS ) {
            if( fgets( buff, sizeof( buff ), fp ) == NULL )
                break;
            if( feof( fp ) || (_isatty( _fileno( fp ) ) && *buff == '\n') )
                break;
            len = strlen( buff );
            if( len > 0 ) {
                if( buff[len - 1] == '\n' ) {
