...
static int MatchFound( char *p )
{
    ...
    if( *word == '(' ) { /* Multiple match words, store them */
        ...
        for( ; MatchWords < 20; ) {
            if( *word == '\0' )
                Fatal( "Missing match word\n" );
            if( _stricmp( word, "\"\"" ) == 0 ) { /* 'No parameter' indicator */
                EmptyOk = true;
            } else {
...
{
    if( _getcwd( includeStk->cwd, sizeof( includeStk->cwd ) ) == NULL ) {
        includeStk->cwd[0] = '\0';
    }
}
