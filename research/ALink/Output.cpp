...
long createOutputSection(char* name, UInt winFlags)
{
    ...
    outlist = (SegmentPtrPtr)checkRealloc(outlist, sizeof(SegmentPtr) * (outcount + 1));
    outlist[outcount] = (SegmentPtr)checkMalloc(sizeof(Segment));
    seglist = (SegmentPtrPtr)checkRealloc(seglist, sizeof(SegmentPtr) * (segcount + 1));
...
void BuildPEImports(long impsectNum, UCharPtr objectTable)
{
    ...
    for (i = 0; i < dllCount; i++)
    {
        ...
        /* add name to table */
        strcpy((CharPtr)impsect->data + namePos - impsect->base, dllNames[i]);
        ...
        for (k = 0; k < reqcount; k++)
        {
            ...
            if (impdefs[reqimps[k]].flags == 0)
            {
                ...
                impNamePos += 2;
                /* store name */
                strcpy((CharPtr)impsect->data + impNamePos - impsect->base,
...
void BuildPEResources(long sectNum, UCharPtr objectTable)
{
    ...
    for (i = 1; i < rescount; i++)
    {
        ...
        for (j = i - 1; j >= 0; j--)
        {
            if (resource[j].typeName)
            {
                ...
                if (wstricmp((const char *)curres.typeName, (const char*)resource[j].typeName) > 0) break;
                if (wstricmp((const char*)curres.typeName, (const char*)resource[j].typeName) == 0)
                {
                    if (resource[j].name)
                    {
    ...
    for (i = 0; i < rescount; i++)
    {
        ...
        {
            nameSize += 2 + 2 * wstrlen((const char*)resource[i].typeName);
        }
        if (resource[i].name)
        {
    ...
    for (i = 0; i < rescount; i++)
    {
        ...
        if ((i > 0) && !wstricmp((const char*)resource[i].typeName, (const char*)resource[i - 1].typeName))
        {
            if (resource[i].name)
            {
    ...
    for (i = 0; i < rescount; i++)
    {
        if (!(resource[i].typeName && curTypeName &&
            !wstricmp((const char*)resource[i].typeName, curTypeName))
            && !(!resource[i].typeName && !curTypeName && curTypeId == resource[i].typeId))
        {
            if (resource[i].typeName)
            {
                ...
                curTypeName = (CharPtr)resource[i].typeName;
            ...
            for (j = i; j < rescount; j++)
            {
                if (resource[i].typeName)
                {
                    ...
                    if (wstricmp((const char *)resource[i].typeName, (const char*)resource[j].typeName) != 0) break;
                }
                else
                {
            ...
            && !(!resource[i].name && !curName && curId == resource[i].id))
        {
            if (resource[i].name)
            {
                ...
                curName = (CharPtr)resource[i].name;
                k = wstrlen(curName);
                ressect->data[namePos] = k & 0xff;
                ressect->data[namePos + 1] = (k >> 8) & 0xff;
                namePos += 2;
