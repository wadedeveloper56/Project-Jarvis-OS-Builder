...
void loadcoff(FILE* objfile)
{
	...
	unsigned long stringOfs;
	unsigned long i, j, k;// , l;
	unsigned long fileStart;
	unsigned long minseg;
	unsigned long numrel;
	...
	if (symbolPtr && numSymbols)
	{
		...
		for (i = 0; i < numSymbols; i++)
		{
			...
			else
			{
				...
				sym[i].name = (unsigned char *)checkStrdup((const char*)(stringList + stringOfs));
			}
			if (!case_sensitive)
			{
			...
			switch (sym[i].clazz)
			{
				case COFF_SYM_SECTION: /* section symbol */
					...
					if (sym[i].section != 0) /* if the section is defined here, make public */
					{
						...
						if (listnode = (SortEntryPtr)binarySearch(publics, pubcount, (char*)sym[i].name))
						{
