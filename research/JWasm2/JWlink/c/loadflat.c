...
/* write the enumerated data pages */
{
    ...
    last_page = 0;
    for( group = Groups; group != NULL; group = group->next_group) {
        DEBUG(( DBG_OLD, "loadflat.WriteDataPages: grp=%s size=%h", group->sym->name , group->size ));
        if( group->size != 0 ) {
            if( last_page != 0 ) {
                if( FmtData.type & (MK_OS2_LE|MK_WIN_VXD) ) {
    ...
    if( last_page == 0 ) {
        ...
        DEBUG(( DBG_OLD, "loadflat.WriteDataPages: PadLoad last_page=%h FmtData.u.os2.segment_shift=%h", last_page, FmtData.u.os2.segment_shift ));
...
/* make an OS/2 flat model executable file */
{
    ...
    DEBUG(( DBG_OLD, "FiniOS2FlatLoadFile() enter" ));
    if ( FmtData.u.pe.os2.segment_shift == -1 )
        FmtData.u.pe.os2.segment_shift = 0;
 ...
 * V1.x -- yuck. */
    ...
    DEBUG(( DBG_OLD, "FiniOS2FlatLoadFile(): WriteDataPages" ));
