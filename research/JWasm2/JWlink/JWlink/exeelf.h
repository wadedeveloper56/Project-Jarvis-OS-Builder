...
#define EI_NIDENT       16
    ...
    typedef struct {
        unsigned_8  e_ident[EI_NIDENT];     //  +0 signature & ID info
        Elf32_Half  e_type;                 // +16 file type (i.e. obj file, exe file)
        Elf32_Half  e_machine;              // +18 required architecture
        Elf32_Word  e_version;              // +20 version of the file
        Elf32_Addr  e_entry;                // +24 program entry point
        Elf32_Off   e_phoff;                // +28 program header offset
        Elf32_Off   e_shoff;                // +32 section header offset
        Elf32_Word  e_flags;                // +36 processor specific flags
        Elf32_Half  e_ehsize;               // +40 elf header size
        Elf32_Half  e_phentsize;            // +42 program header entry size
        Elf32_Half  e_phnum;                // +44 number of program header entries
        Elf32_Half  e_shentsize;            // +46 section header entry size
...
// program header
    ...
    typedef struct {
        Elf32_Word  p_type;         //  +0 type of segment
        Elf32_Off   p_offset;       //  +4 offset of segment from beginnning of file
        Elf32_Addr  p_vaddr;        //  +8 segment virtual address
        Elf32_Addr  p_paddr;        // +12 segment physical address
        Elf32_Word  p_filesz;       // +16 size of segment in file
        Elf32_Word  p_memsz;        // +20 size of segment in memory
        Elf32_Word  p_flags;        // +24
        Elf32_Word  p_align;        // +28 segment align value (in mem & file)
    } Elf32_Phdr;
