...
orl_return CoffCreateSymbolHandles( coff_file_handle file_hnd )
{
    ...
    file_hnd->symbol_handles = _ClientAlloc(coff_symbol_handle, file_hnd, sizeof( coff_symbol_handle_struct ) * file_hnd->num_symbols );
    ...
    for( loop = 0; loop < file_hnd->num_symbols; loop++ ) {
        ...
        if( current->symbol->name.non_name.zeros == 0 ) {
            ...
            if( strlen( current->symbol->name.name_string ) >= COFF_SYM_NAME_LEN ) {
                current->name = _ClientAlloc(char *, file_hnd, COFF_SYM_NAME_LEN + 1 );
        ...
        switch( current->symbol->sec_num ) {
            case IMAGE_SYM_DEBUG:
                current->type = (orl_symbol_type)(current->type|ORL_SYM_TYPE_DEBUG);
            ...
            case IMAGE_SYM_UNDEFINED:
                ...
                break;
            default:
                current->type = (orl_symbol_type)(current->type | ORL_SYM_TYPE_DEFINED);
