...
/**************************************************************/
{
    ...
    if( flags & IMAGE_FILE_RELOCS_STRIPPED ) {
            coff_file_hnd->flags = (orl_file_flags)(coff_file_hnd->flags | ORL_FILE_FLAG_RELOCS_STRIPPED);
    }
    if( flags & IMAGE_FILE_LINE_NUMS_STRIPPED ) {
        coff_file_hnd->flags = (orl_file_flags)(coff_file_hnd->flags | ORL_FILE_FLAG_LINE_NUMS_STRIPPED);
    }
    if( flags & IMAGE_FILE_LOCAL_SYMS_STRIPPED ) {
        coff_file_hnd->flags = (orl_file_flags)(coff_file_hnd->flags | ORL_FILE_FLAG_LOCAL_SYMS_STRIPPED);
    }
    if( flags & IMAGE_FILE_DEBUG_STRIPPED ) {
        coff_file_hnd->flags = (orl_file_flags)(coff_file_hnd->flags | ORL_FILE_FLAG_DEBUG_STRIPPED);
    }
    if( flags & IMAGE_FILE_16BIT_MACHINE ) {
        coff_file_hnd->flags = (orl_file_flags)(coff_file_hnd->flags | ORL_FILE_FLAG_16BIT_MACHINE);
    }
    if( flags & IMAGE_FILE_32BIT_MACHINE ) {
        coff_file_hnd->flags = (orl_file_flags)(coff_file_hnd->flags | ORL_FILE_FLAG_32BIT_MACHINE);
    }
    if( flags & IMAGE_FILE_SYSTEM ) {
        coff_file_hnd->flags = (orl_file_flags)(coff_file_hnd->flags | ORL_FILE_FLAG_SYSTEM);
    }
    if( coff_file_hnd->type != ORL_FILE_TYPE_OBJECT ) {
         ...
         */
        coff_file_hnd->flags = (orl_file_flags)(coff_file_hnd->flags | ORL_FILE_FLAG_LITTLE_ENDIAN);
    } else {
        /*
        ...
        */
        if( flags & IMAGE_FILE_BYTES_REVERSED_HI ) {
            coff_file_hnd->flags = (orl_file_flags)(coff_file_hnd->flags | ORL_FILE_FLAG_BIG_ENDIAN);
        } else {
...
/****************************************************************/
{
    ...
    if( s_hdr->flags & IMAGE_SCN_LNK_INFO ) {
        ...
        coff_sec_hnd->flags = (orl_sec_flags)(coff_sec_hnd->flags | ORL_SEC_FLAG_EXEC);
    } else if( s_hdr->flags & IMAGE_SCN_CNT_INITIALIZED_DATA ) {
        coff_sec_hnd->type = ORL_SEC_TYPE_PROG_BITS;
        coff_sec_hnd->flags = (orl_sec_flags)(coff_sec_hnd->flags | ORL_SEC_FLAG_INITIALIZED_DATA);
    } else if( s_hdr->flags & IMAGE_SCN_CNT_UNINITIALIZED_DATA ) {
        coff_sec_hnd->type = ORL_SEC_TYPE_PROG_BITS;
        coff_sec_hnd->flags = (orl_sec_flags)(coff_sec_hnd->flags | ORL_SEC_FLAG_UNINITIALIZED_DATA);
    } else {
...
//  }
    if( s_hdr->flags & IMAGE_SCN_TYPE_NO_PAD ) {
        coff_sec_hnd->flags = (orl_sec_flags)(coff_sec_hnd->flags | ORL_SEC_FLAG_NO_PADDING);
    }
...
//  }
    if( s_hdr->flags & IMAGE_SCN_LNK_REMOVE ) {
        coff_sec_hnd->flags = (orl_sec_flags)(coff_sec_hnd->flags | ORL_SEC_FLAG_REMOVE);
    }
    if( s_hdr->flags & IMAGE_SCN_LNK_COMDAT ) {
        coff_sec_hnd->flags = (orl_sec_flags)(coff_sec_hnd->flags | ORL_SEC_FLAG_COMDAT);
    }
    if( s_hdr->flags & IMAGE_SCN_MEM_DISCARDABLE ) {
        coff_sec_hnd->flags = (orl_sec_flags)(coff_sec_hnd->flags | ORL_SEC_FLAG_DISCARDABLE);
    }
    if( s_hdr->flags & IMAGE_SCN_MEM_NOT_CACHED ) {
        coff_sec_hnd->flags = (orl_sec_flags)(coff_sec_hnd->flags | ORL_SEC_FLAG_NOT_CACHED);
    }
    if( s_hdr->flags & IMAGE_SCN_MEM_NOT_PAGED ) {
        coff_sec_hnd->flags = (orl_sec_flags)(coff_sec_hnd->flags | ORL_SEC_FLAG_NOT_PAGEABLE);
    }
    if( s_hdr->flags & IMAGE_SCN_MEM_SHARED ) {
        coff_sec_hnd->flags = (orl_sec_flags)(coff_sec_hnd->flags | ORL_SEC_FLAG_SHARED);
    }
    if( s_hdr->flags & IMAGE_SCN_MEM_EXECUTE ) {
        coff_sec_hnd->flags = (orl_sec_flags)(coff_sec_hnd->flags | ORL_SEC_FLAG_EXECUTE_PERMISSION);
    }
    if( s_hdr->flags & IMAGE_SCN_MEM_READ ) {
        coff_sec_hnd->flags = (orl_sec_flags)(coff_sec_hnd->flags | ORL_SEC_FLAG_READ_PERMISSION);
    }
    if( s_hdr->flags & IMAGE_SCN_MEM_WRITE ) {
        coff_sec_hnd->flags = (orl_sec_flags)(coff_sec_hnd->flags | ORL_SEC_FLAG_WRITE_PERMISSION);
    }
...
/**********************************************************************/
{
    ...
    if( coff_file_hnd->num_sections == 0 ) {
        ...
        reloc_sec_offset = _ClientAlloc(orl_file_offset *, coff_file_hnd, sizeof( orl_file_offset ) * coff_file_hnd->num_sections );
        if( !(reloc_sec_offset) ) return( ORL_OUT_OF_MEMORY );
        reloc_sec_size = _ClientAlloc(orl_sec_size *, coff_file_hnd, sizeof( orl_sec_size ) * coff_file_hnd->num_sections );
        if( !(reloc_sec_size) ) {
        ...
        coff_file_hnd->orig_sec_hnd = _ClientAlloc(coff_sec_handle *, coff_file_hnd, sizeof( coff_sec_handle ) * coff_file_hnd->num_sections );
    ...
    for( loop = 0; loop < coff_file_hnd->num_sections; loop++ ) {
        coff_sec_hnd = _ClientAlloc(coff_sec_handle, coff_file_hnd, sizeof( coff_sec_handle_struct ) );
        if( !coff_sec_hnd ) {
        ...
        if( s_hdr->name[0] != '/' ) {
            coff_sec_hnd->name = _ClientAlloc(char*, coff_file_hnd, COFF_SEC_NAME_LEN + 1 );
    ...
    coff_file_hnd->coff_sec_hnd = _ClientAlloc(coff_sec_handle *, coff_file_hnd, sizeof( coff_sec_handle ) * (coff_file_hnd->num_sections + num_reloc_secs + 2) );
    ...
    for( loop = 0; loop < coff_file_hnd->num_sections; loop++ ) {
        if( reloc_sec_size[loop] > 0 ) {
            ...
            coff_file_hnd->coff_sec_hnd[coff_file_hnd->num_sections + reloc_secs_created] = _ClientAlloc(coff_sec_handle, coff_file_hnd, sizeof( coff_sec_handle_struct ) );
    ...
    coff_file_hnd->symbol_table = _ClientAlloc(coff_sec_handle, coff_file_hnd, sizeof( coff_sec_handle_struct ) );
    ...
    coff_file_hnd->string_table = _ClientAlloc(coff_sec_handle, coff_file_hnd, sizeof( coff_sec_handle_struct ) );
