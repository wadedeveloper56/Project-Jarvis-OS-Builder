...
{
    c_file->string_table = _ClientAlloc(char*, c_file->coff_file_hnd, INIT_MAX_SIZE_COFF_STRING_TABLE );
...
static void AddCoffString( coff_lib_file  *c_file, char *name, int len )
{
    ...
    if( ( c_file->string_table_size + len ) >= c_file->max_string_table_size ) {
        ...
        x = _ClientAlloc(char*, c_file->coff_file_hnd, c_file->max_string_table_size );
        if( x == NULL )
            return;
...
static int DataImpLibInit( coff_file_handle coff_file_hnd )
{
    if (coff_file_hnd->implib_data == NULL) {
        coff_file_hnd->implib_data = _ClientAlloc(char *, coff_file_hnd, sizeof( int ) + sizeof( long ) );
        if( coff_file_hnd->implib_data == NULL )
...
static int AddDataImpLib( coff_file_handle coff_file_hnd, void * buff, int len )
{
    ...
    x = _ClientAlloc(char*, coff_file_hnd, IMPLIB_LEN + len );
    if( x == NULL )
        return( ORL_OUT_OF_MEMORY );
...
static int CoffCreateImport( coff_file_handle coff_file_hnd, import_sym * import )
{
    ...
    DLLSymbolName = (char*)alloca(symbol_name_len + 1 );
    ...
    buffer = (char *)alloca(max( strlen(import->DLLName), symbol_name_len) + 64 );
...
/* .idata$5 section header */
    if( import->type == IMPORT_OBJECT_ORDINAL) {
        ...
        section_no = AddCoffSection( &c_file, ".idata$5", sizeof(bnull.b32), 0, IMAGE_SCN_ALIGN_4BYTES | IMAGE_SCN_LNK_COMDAT | IMAGE_SCN_CNT_INITIALIZED_DATA | IMAGE_SCN_MEM_READ );
    } else {
        ...
        section_no = AddCoffSection( &c_file, ".idata$5", sizeof(bnull.b32), 1, IMAGE_SCN_ALIGN_4BYTES | IMAGE_SCN_LNK_COMDAT | IMAGE_SCN_CNT_INITIALIZED_DATA | IMAGE_SCN_MEM_READ );
    }
    AddCoffSymSec( &c_file, IMAGE_COMDAT_SELECT_NODUPLICATES, section_no );
...
/* .idata$4 section header */
    if( import->type == IMPORT_OBJECT_ORDINAL) {
        ...
        section_no = AddCoffSection( &c_file, ".idata$4", sizeof(bnull.b32), 0, IMAGE_SCN_ALIGN_4BYTES | IMAGE_SCN_LNK_COMDAT | IMAGE_SCN_CNT_INITIALIZED_DATA | IMAGE_SCN_MEM_READ );
        AddCoffSymSec( &c_file, IMAGE_COMDAT_SELECT_ASSOCIATIVE, section_no );
    } else {
        ...
        section_no = AddCoffSection( &c_file, ".idata$4", sizeof(bnull.b32), 1, IMAGE_SCN_ALIGN_4BYTES | IMAGE_SCN_LNK_COMDAT | IMAGE_SCN_CNT_INITIALIZED_DATA | IMAGE_SCN_MEM_READ );
        AddCoffSymSec( &c_file, IMAGE_COMDAT_SELECT_ASSOCIATIVE, section_no );
/* .idata$6 section header */
        ...
        section_no = AddCoffSection( &c_file, ".idata$6", sizeof(ordinal) + ( dllsymbol_name_len | 1 ) + 1, 0, IMAGE_SCN_ALIGN_2BYTES | IMAGE_SCN_LNK_COMDAT | IMAGE_SCN_CNT_INITIALIZED_DATA | IMAGE_SCN_MEM_READ );
...
int convert_import_library(coff_file_handle coff_file_hnd)
{
    ...
    sym.exportedName = (char*)coff_file_hnd->coff_hnd->funcs->read( coff_file_hnd->file, i_hdr->size_of_data );
...
orl_funcs ImportLibData = {ImportLibRead, ImportLibSeek, NULL };
