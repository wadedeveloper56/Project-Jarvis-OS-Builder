...
void BuildPEResources(long sectNum, PUCHAR objectTable)
{
	...
	for (i = 1; i < rescount; i++)
	{
		...
		for (j = i - 1; j >= 0; j--)
		{
			if (resource[j].typename1)
			{
				...
				if (wstricmp((const char*)curres.typename1, (const char*)resource[j].typename1) > 0) break;
				if (wstricmp((const char*)curres.typename1, (const char*)resource[j].typename1) == 0)
				{
					if (resource[j].name)
					{
	...
	for (i = 0; i < rescount; i++)
	{
		...
		{
			nameSize += 2 + 2 * wstrlen((const char*)resource[i].typename1);
		}
		if (resource[i].name)
		{
	...
	for (i = 0; i < rescount; i++)
	{
		...
		if ((i > 0) && !wstricmp((const char*)resource[i].typename1, (const char*)resource[i - 1].typename1))
		{
			if (resource[i].name)
			{
	...
	tableSize = (rescount + numtypes + numPairs) * PE_RESENTRY_SIZE + (numtypes + numPairs + 1) * PE_RESDIR_SIZE;
	...
	for (i = 0; i < rescount; i++)
	{
		if (!(resource[i].typename1 && curTypeName &&
			!wstricmp((const char*)resource[i].typename1, curTypeName))
			&& !(!resource[i].typename1 && !curTypeName && curTypeId == resource[i].typeid1))
		{
			...
			for (j = i; j < rescount; j++)
			{
				if (resource[i].typename1)
				{
					...
					if (wstricmp((const char*)resource[i].typename1, (const char*)resource[j].typename1) != 0) break;
				}
				else
				{
