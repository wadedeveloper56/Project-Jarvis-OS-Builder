    ...
    for( sfile = FileTable.first; sfile != NULL; sfile = sfile->next ) {
        ...
        case WL_LTYPE_AR:
            ...
            if( Options.ar_libformat == AR_FMT_BSD ) {
				...
				/* jwlink: don't store short names in the "longnames" member */
                if( name_length < AR_NAME_LEN ) {
                    sfile->name_offset = -1;
                    name_length = 0;
                } else {
...
void WriteFile2( sym_file *sfile )
/*******************************/
{
    libfile     io;
...
/**************************************/
{
    ...
    SortSymbols();
    DEBUG(("WriteArMlibFileTable, NumSymbols=%u NumFiles=%u TotalNameLength=%u\n", NumSymbols, NumFiles, TotalNameLength ));
    ...
    case WL_LTYPE_AR:
        ...
        default:
            ...
            /* jwlink */
            //if( TotalNameLength > 0 ) {
            if( TotalNameLength > 0 || Options.ar_libformat == AR_FMT_COFF ) {
                header_size += AR_HEADER_SIZE + RoundWord( TotalNameLength );
            }
    ...
    arch.mode = 0;
    if( dict1_size > 0 ) {
		DEBUG(("WriteArMlibFileTable: writing 1. directory\n" ));
    ...
    if( dict2_size > 0 ) {
		DEBUG(("WriteArMlibFileTable: writing 2. directory\n" ));
	...
	/* jwlink: for COFF, always write the "longnames" member,
	 * even if it's empty
	 */
    //if( TotalNameLength > 0 ) {
    if( TotalNameLength > 0 || Options.ar_libformat == AR_FMT_COFF ) {
		...
		DEBUG(("WriteArMlibFileTable: writing string table\n" ));
    ...
    if( Options.libtype == WL_LTYPE_MLIB ) {
		DEBUG(("WriteArMlibFileTable: writing full filename table\n" ));
